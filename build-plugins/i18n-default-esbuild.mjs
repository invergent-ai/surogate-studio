...
const i18nSourceDir = join(__dirname, 'src/main/webapp/i18n/default');
...
export default {
  ...
  async setup(build) {
    ...
    build.onResolve({ filter: /^i18n\/default\// }, ({ path }) => {
      return { namespace: 'i18n-json', path };
    });

    build.onLoad({ filter: /^i18n\/default\//, namespace: 'i18n-json' }, async ({ path }) => {
      const match = /^i18n\/default\/(?<lang>.*)\.*.json/.exec(path);
