...
import net.statemesh.security.UserNotActivatedException;
import net.statemesh.service.exception.*;
import net.statemesh.service.lakefs.LakeFsNoChangesException;
import net.statemesh.web.rest.vm.FieldErrorVM;
...
public class ExceptionTranslator extends ResponseEntityExceptionHandler {
    ...
    private String getMappedMessageKey(Throwable err) {
        if (err instanceof MethodArgumentNotValidException) {
            ...
            return ErrorConstants.ERR_BAD_CREDENTIALS;
        } else if (err instanceof UserNotActivatedException || err.getCause() instanceof UserNotActivatedException) {
            return ErrorConstants.ERR_NOT_ACTIVATED;
        } else if (err instanceof ConcurrencyFailureException || err.getCause() instanceof ConcurrencyFailureException) {
            return ErrorConstants.ERR_CONCURRENCY_FAILURE;
        } else if (err instanceof ProjectHasApplicationsException) {
            return ErrorConstants.ERR_PROJECT_HAS_APPLICATION;
        }
        return null;
    }
    ...
    private String getCustomizedTitle(Throwable err) {
        ...
        if (err instanceof LakeFsNoChangesException) return "Could not commit: no changes";
        return null;
    }
    ...
    private HttpStatus getMappedStatus(Throwable err) {
        ...
        if (err instanceof BadHeadersException) return HttpStatus.BAD_REQUEST;
        if (err instanceof RateLimitException) return HttpStatus.TOO_MANY_REQUESTS;
        if (err instanceof OnboardingException) return HttpStatus.INTERNAL_SERVER_ERROR;
        if (err instanceof BadPrerequisitesException) return HttpStatus.EXPECTATION_FAILED;
        if (err instanceof NoZoneAvailableException) return HttpStatus.REQUESTED_RANGE_NOT_SATISFIABLE;
        return null;
    }
    ...
    private HttpHeaders buildHeaders(Throwable err) {
        return err instanceof BadRequestAlertException badRequestAlertException
            ...
            : HeaderUtil
            .createFailureAlert(applicationName, false, "", "", err.getMessage());
    }
    ...
    private boolean containsPackageName(String message) {
        ...
        return StringUtils.containsAny(message, "org.", "java.", "net.", "jakarta.", "javax.", "com.", "io.", "de.", "net.statemesh");
    }
}
