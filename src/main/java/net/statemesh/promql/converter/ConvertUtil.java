...
import net.statemesh.promql.converter.query.QueryDataType;
import net.statemesh.promql.converter.series.DefaultSeriesResult;
import net.statemesh.promql.converter.series.SeriesResultItem;
import net.statemesh.promql.converter.status.DefaultConfigResult;
import net.statemesh.promql.converter.target.DefaultTargetResult;
import net.statemesh.promql.converter.target.TargetResultItem;
...
//							System.out.println(seriesData);
	...
	@SuppressWarnings("unchecked")
	public static <T extends MetricData> DefaultQueryResult<T> convertQueryResultString(String jsonString) {
			...
			public DefaultQueryResult<T> read(JsonReader reader) throws IOException {
				...
				while(reader.hasNext()) {
					...
					if("status".equals(propertyName)) {
						...
						reader.endObject();
					} else if ("stats".equals(propertyName)) {
                        reader.beginObject();
                        while(reader.hasNext()) {
                            reader.nextName();
                            reader.nextString();
