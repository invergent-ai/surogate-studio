...
@Component({
  ...
  template: `
    <div class="flex flex-column gap-4">
      <div>
        <code-editor language="SQL" [(ngModel)]="code"
                     [languages]="languages" placeholder="DuckDB SQL..."></code-editor>
        <div class="flex justify-content-end">
          <p class="font-italic text-xs">Powered by <a href="https://duckdb.org/2021/10/29/duckdb-wasm.html" target="_blank">DuckDB-WASM</a>. For a full SQL reference, see the <a href="https://duckdb.org/docs/stable/sql/statements/select.html" target="_blank">DuckDB Documentation</a></p>
        </div>
      </div>
      ...
      <table *ngIf="!isLoading()" class="table table-striped w-full" style="border-collapse: collapse">
        <thead>
        <th *ngFor="let col of columns()" style="text-align: left; border-collapse: collapse" class="surface-100 p-1 border-1">{{col}}</th>
        </thead>
        <tbody>
