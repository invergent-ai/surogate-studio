...
@Component({
  selector: 'sm-commit',
  standalone: true,
  imports: [TableModule, Button, CardModule, SideBySideDiffComponent],
...
export class CommitDetailsComponent {
  ...
  commit = derivedAsync(
    () =>
      this.commitId() &&
      this.repoId() &&
      this.lakeFsService.getCommit(this.repoId(), this.commitId()).pipe(catchError(e => displayErrorAndRethrow(this.store, e))),
  );
  diffs = derivedAsync(
    () =>
      this.commit() &&
      this.lakeFsService
        .diff(this.repoId(), this.commit().parents[0], this.commit().id)
        .pipe(catchError(e => displayErrorAndRethrow(this.store, e))),
  );

  selectedDiff = signal<ILakeFsDiff>(null);
  shouldGetLeftStat = computed(
    () =>
      this.selectedDiff() &&
      (this.selectedDiff().type === 'CHANGED' || this.selectedDiff().type === 'CONFLICT' || this.selectedDiff().type === 'REMOVED'),
  );
  shouldGetRightStat = computed(
    () =>
      this.selectedDiff() &&
      (this.selectedDiff().type === 'CHANGED' || this.selectedDiff().type === 'CONFLICT' || this.selectedDiff().type === 'ADDED'),
  );
  leftStat = derivedAsync(
    () =>
      this.shouldGetLeftStat() &&
      this.lakeFsService
        .getStat(this.repoId(), this.commit().parents[0], this.selectedDiff().path)
        .pipe(catchError(e => displayErrorAndRethrow(this.store, e))),
  );
  rightStat = derivedAsync(
    () =>
      this.shouldGetRightStat() &&
      this.lakeFsService
        .getStat(this.repoId(), this.commit().id, this.selectedDiff().path)
        .pipe(catchError(e => displayErrorAndRethrow(this.store, e))),
  );
  leftContent = derivedAsync(
    () => this.leftStat() && this.lakeFsService.fetchObjectAsText(this.repoId(), this.commit().parents[0], this.selectedDiff().path),
  );
  rightContent = derivedAsync(
    () => this.rightStat() && this.lakeFsService.fetchObjectAsText(this.repoId(), this.commit().id, this.selectedDiff().path),
  );
